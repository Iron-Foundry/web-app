...
function SuggestionsPage() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/feedback?type=suggestion`, { headers: getAuthHeaders() })
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="font-rs-bold text-4xl text-primary">Suggestions</h1>
        <p className="text-sm text-muted-foreground">Share ideas to improve the clan.</p>
      </div>

      <Separator />

      <h2 className="text-sm font-semibold">All Suggestions</h2>
          ...
          {items.map((item) =>
            editingId === item.id ? (
              <div key={item.id} className="col-span-full border rounded-lg p-4 space-y-3 max-w-2xl">
                <h3 className="text-sm font-semibold">Edit Suggestion</h3>
                <Input
